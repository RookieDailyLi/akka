...
class EventSourcedBehaviorLoggingSpec
  ...
  "Chatty behavior" must {
    val myId = PersistenceId.ofUniqueId("chat-1")
    ...
    "log internal messages in 'Internal' logger without logging user data (Persist)" in {
      LoggingTestKit
        ...
        .withLoggerName("akka.persistence.typed.EventSourcedBehaviorLoggingSpec$ChattyEventSourcingBehavior$Internal")
        .expect {
    ...
    "Internal logger preserves MDC source" in {
      LoggingTestKit
        ...
        .withLoggerName("akka.persistence.typed.EventSourcedBehaviorLoggingSpec$ChattyEventSourcingBehavior$Internal")
        .withMdc(Map("persistencePhase" -> "running-cmd", "persistenceId" -> "chat-1"))
        .expect {
          chattyActor ! Hello("Mary")
        }
    }
